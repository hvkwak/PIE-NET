...
def placeholder_inputs_stage_3(batch_size,num_point):
    ...
    proposal_pl = tf.compat.v1.placeholder(tf.int32,shape=(batch_size,num_point))
    dof_regression_pl = tf.compat.v1.placeholder(tf.float32,shape=(batch_size,1,6))
    field_pl = tf.compat.v1.placeholder(tf.float32,shape=(batch_size,3,num_point,6))
...
def get_stage_3(pointclouds_pl,field_pl,is_training,bn_decay=None):
    ...
    feat = tf.reduce_max(input_tensor=feat,axis = 1)
...
def get_stage_3_loss(pred_proposal,pred_dof_regression,proposal_pl,dof_regression_pl):
    ...
    loss1 = tf.reduce_mean(input_tensor=tf.nn.sparse_softmax_cross_entropy_with_logits(logits = pred_proposal,labels = proposal_pl))
    loss2 = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=smooth_l1_dist(pred_dof_regression-dof_regression_pl),axis=2))
    task_1_acc = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.cast(tf.equal(tf.argmax(input=pred_proposal,axis=2,output_type = tf.int32),\
                          proposal_pl),tf.float32),axis = 1)/num_point)
    proposal = tf.argmax(input=pred_proposal,axis=2,output_type = tf.int32)
    ...
    iou = tf.reduce_sum(input_tensor=tf.cast(tf.logical_and(proposal, proposal_pl), tf.float32), axis=-1) / \
                      (tf.reduce_sum(input_tensor=tf.cast(tf.logical_or(proposal, proposal_pl), tf.float32), axis=-1) + epsilon)
    iou = tf.reduce_mean(input_tensor=iou)
    loss = loss1+loss2
