...
def tf_BSpline_function(input):
    #num_knots = 7
    cyclical = False
    degree = 3
    #max_pos = 1.0
    knots = input
    positions = tf.expand_dims(tf.range(start=0.0, limit=1.0, delta=0.01, dtype=knots.dtype), axis=-1)
...
def BSpline_fn(inp):
    ...
    t=np.append(t,[1,1,1])
    tck=[t,[x,y, z],3]
    u3 = np.linspace(0, 1, 64)
    return np.stack(interpolate.splev(u3, tck), axis = 1).astype(np.float32)


@tf.function(input_signature = [tf.TensorSpec(None, tf.float32)])
def tf_BSpline_function(input):
    return tf.numpy_function(BSpline_fn, [input], tf.float32)
'''
...
def placeholder_inputs_32(batch_size):
    ...
    sample_points_pl = tf.compat.v1.placeholder(tf.float32, shape = (batch_size*256, SN, 3))
    labels_256_sn = tf.compat.v1.placeholder(tf.int32, shape = (batch_size*256, SN))
    ...
    mask_256_sn_candidates = tf.compat.v1.placeholder(tf.int32, shape = (batch_size*256, SN))    
    mask_256_1_if_edge = tf.compat.v1.placeholder(tf.int32, shape = (batch_size*256, 1))
    mask_256_1_if_proposed = tf.compat.v1.placeholder(tf.int32, shape = (batch_size*256, 1))

    #sample_line_points = tf.compat.v1.placeholder(tf.float32, shape = (batch_size*256, 100, 3))
    return sample_points_pl, labels_256_sn, labels_type, labels_type_one_hot, mask_256_sn_candidates, mask_256_1_if_edge, mask_256_1_if_proposed
...
# get_model_32_PointNetpp
'''
def get_model_32(point_cloud, is_training, STAGE=2, bn_decay=None):
...
#    l0_points = pointnet_fp_module(l0_xyz, l1_xyz, tf.concat([l0_xyz,l0_points],axis=-1), l1_points, [128,128,128], is_training, bn_decay, scope='pointnet/fa_layer3')
    ...
    # Layer 1
    l1_xyz, l1_points, l1_indices = pointnet_sa_module(l0_xyz, l0_points, npoint=64, radius=0.05, nsample=32, mlp=[32,32,64], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='stage2/layer1')
    l2_xyz, l2_points, l2_indices = pointnet_sa_module(l1_xyz, l1_points, npoint=32, radius=0.1, nsample=32, mlp=[64,64,128], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='stage2/layer2')
    l3_xyz, l3_points, l3_indices = pointnet_sa_module(l2_xyz, l2_points, npoint=16, radius=0.2, nsample=32, mlp=[128,128,256], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='stage2/layer3')
    l4_xyz, l4_points, l4_indices = pointnet_sa_module(l3_xyz, l3_points, npoint=8, radius=0.4, nsample=32, mlp=[256,256,512], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='stage2/layer4')

    # Feature Propagation layers
    l3_points = pointnet_fp_module(l3_xyz, l4_xyz, l3_points, l4_points, [256,256], is_training, bn_decay, scope='stage2/fa_layer1')
    l2_points = pointnet_fp_module(l2_xyz, l3_xyz, l2_points, l3_points, [256,256], is_training, bn_decay, scope='stage2/fa_layer2')
    l1_points = pointnet_fp_module(l1_xyz, l2_xyz, l1_points, l2_points, [256,128], is_training, bn_decay, scope='stage2/fa_layer3')
    l0_points = pointnet_fp_module(l0_xyz, l1_xyz, l0_points, l1_points, [128,128,128], is_training, bn_decay, scope='stage2/fa_layer4')

    # FC layers
    net = tf_util.conv1d(l0_points, 128, 1, padding='VALID', bn=True, is_training=is_training, scope='stage2/pointnet/fc1', bn_decay=bn_decay)

    end_points['feats'] = net
    if STAGE == 2:
        net = tf_util.dropout(net, keep_prob=0.5, is_training=is_training, scope='stage2/pointnet/dp1')
    #if is_training:
    #    net = tf_util.dropout(net, keep_prob=0.5, is_training=is_training, scope='pointnet/dp1')        
    
    # dof_feature
    dof_feat = tf_util.conv1d(net, 128, 1, padding='VALID', bn=True, is_training=is_training, scope='stage2/pointnet/fc_dof', bn_decay=bn_decay)
    # simmat_feature
    ...
    feat3_1_1 = tf_util.conv1d(dof_feat,128,1,padding='VALID',activation_fn = None,scope = 'stage2/feat3_1_1/fc1', bn_decay=bn_decay)
    pred_open_curve_seg = tf_util.conv1d(feat3_1_1, 2, 1, padding='VALID', activation_fn=None, scope='stage2/feat3_1_1/fc2', bn_decay=bn_decay)
    
    # Corner Points Classification
    feat3_1_2 = tf.reshape(dof_feat, [batch_size, -1])
    class_head = tf_util.fully_connected(feat3_1_2, 128, bn=True, is_training=is_training, scope='stage2/feat3_1_2/fc1', bn_decay=bn_decay)
    class_head = tf_util.fully_connected(class_head, 64, bn=True, is_training=is_training, scope='stage2/feat3_1_2/fc2', bn_decay=bn_decay)
    pred_open_curve_cls = tf_util.fully_connected(class_head, 4, bn=True, is_training=is_training, scope='stage2/feat3_1_2/fc3', bn_decay=bn_decay)
    
    #feat3_1_2 = tf_util.conv1d(dof_feat,128,1,padding='VALID',activation_fn = None,scope = 'stage2/feat3_1_2/fc1', bn_decay=bn_decay)
    #pred_open_curve_cls = tf_util.conv2d(feat3_1_2, 4, [128, 128], padding='VALID', activation_fn=None, scope='stage2/feat3_1_2/fc2', bn_decay=bn_decay)
    #pred_open_curve_cls = tf_util.fully_connected(feat3_1_2, 4, activation_fn=None, scope='stage2/feat3_1_2/fc2')
    ...
    #feat3_1_31 = tf_util.conv1d(feat3_1_3,128,1,padding='VALID',activation_fn = None,scope = 'stage1/feat3_1_3/fc2', bn_decay=bn_decay)
    ...
    #feat3_1_41 = tf_util.conv1d(feat3_1_4,128,1,padding='VALID',activation_fn = None,scope = 'stage1/feat3_1_4/fc2', bn_decay=bn_decay)
...
#                                             pred_labels_type,pred_simmat,pred_conf_logits
    return pred_open_curve_seg, pred_open_curve_cls
    #return pred_labels_edge_p, pred_labels_corner_p, pred_reg_edge_p, pred_reg_corner_p
'''

# note that this is PointNet without T-Net
...
def get_model_32(point_cloud, is_training, bn_decay=None):
    ...
    #pred_open_curve_reg_BSpline = tf_util.fully_connected(regression_head, 21, activation_fn=None, scope='stage2/reg/fc3_0') # 7 coordinates
...
def get_model_32(point_cloud, is_training, bn_decay=None):
    ...
    pred_open_curve_reg_BSpline = tf_util.fully_connected(regression_head, 21, activation_fn=None, scope='stage2/reg/fc3_0') # 3 coordinates
...
def get_stage_2_loss(pred_open_curve_seg, \
    ...
    num_corner_pairs = batch_mask_256_sn_if_edge.get_shape()[0] ## 256

    # Change this accordingly. make sure that loss balancing takes place.
    mask_256_sn_if_proposed = tf.cast(batch_mask_256_sn_if_proposed, tf.float32)
    neg_mask_256_sn_if_proposed = tf.ones_like(mask_256_sn_if_proposed) - mask_256_sn_if_proposed
    Np_256_sn_if_proposed = tf.expand_dims(tf.reduce_sum(mask_256_sn_if_proposed, axis=1),1) + 0.001
    Nn_256_sn_if_proposed = tf.expand_dims(tf.reduce_sum(neg_mask_256_sn_if_proposed, axis=1),1)

    mask_256_sn_if_edge = tf.cast(batch_mask_256_sn_if_edge, tf.float32)
    neg_mask_256_sn_if_edge = tf.ones_like(mask_256_sn_if_edge) - mask_256_sn_if_edge
    Np_256_sn_if_edge = tf.expand_dims(tf.reduce_sum(mask_256_sn_if_edge, axis=1),1) + 0.001
    Nn_256_sn_if_edge = tf.expand_dims(tf.reduce_sum(neg_mask_256_sn_if_edge, axis=1),1)


    #print_op2 = tf.compat.v1.print("Debug output mask_256_sn:", mask_256_sn, mask_256_sn.shape)
    #with tf.control_dependencies([print_op2]):
    #    mask_256_sn = tf.compat.v1.identity(mask_256_sn)

    # pair masks if they are corners or proposed:
    #mask_1_if_corner = tf.cast(batch_mask_256_1_if_edge, tf.float32)
    ...
    mask_256_1_if_edge = tf.cast(batch_mask_256_1_if_edge, tf.float32)
    neg_mask_256_1_if_edge = tf.ones_like(mask_256_1_if_edge) - mask_256_1_if_edge
    Np_256_1_if_edge = tf.reduce_sum(mask_256_1_if_edge) + 0.001
    Nn_256_1_if_edge = tf.reduce_sum(neg_mask_256_1_if_edge)
    

    mask_256_1_if_proposed = tf.cast(batch_mask_256_1_if_proposed, tf.float32)
    neg_mask_256_1_if_proposed = tf.ones_like(mask_256_1_if_proposed) - mask_256_1_if_proposed
    Np_256_1_if_proposed = tf.reduce_sum(mask_256_1_if_proposed) + 0.001
    Nn_256_1_if_proposed = tf.reduce_sum(neg_mask_256_1_if_proposed)
    #(mask_256_sn_if_proposed*(Nn_256_sn_if_proposed/Np_256_sn_if_proposed)+1)
    #(mask_256_1_if_proposed*(Nn_256_1_if_proposed/Np_256_1_if_proposed)+1)
    seg_entropy_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = pred_open_curve_seg, labels = batch_mask_256_sn_if_edge)
    #seg_entropy_loss_valid_256_sn = seg_entropy_loss*((mask_256_sn_if_proposed*(Nn_256_sn_if_proposed/Np_256_sn_if_proposed)+1)*(mask_256_sn_if_edge*(Nn_256_sn_if_edge/Np_256_sn_if_edge)+1))
    seg_entropy_loss_valid_256_sn = seg_entropy_loss*((mask_256_sn_if_proposed*(Nn_256_sn_if_proposed/Np_256_sn_if_proposed)+1))
    #seg_entropy_loss_valid_256_sn = seg_entropy_loss*((mask_256_sn_if_proposed*(Nn_256_sn_if_proposed/Np_256_sn_if_proposed)+1)*(mask_256_sn_if_edge*(Nn_256_sn_if_edge/Np_256_sn_if_edge)+1))
    #seg_3_2_loss = tf.reduce_mean(seg_entropy_loss_valid_256_sn*(mask_256_1_if_edge*(Nn_256_1_if_edge/Np_256_1_if_edge)+1))
    #seg_3_2_loss = tf.reduce_mean(seg_entropy_loss_valid_256_sn*(mask_256_1_if_proposed*(Nn_256_1_if_proposed/Np_256_1_if_proposed)+1))
    seg_3_2_loss = tf.reduce_mean(seg_entropy_loss_valid_256_sn*(mask_256_1_if_proposed*(Nn_256_1_if_proposed/Np_256_1_if_proposed)+1))
    
    #seg_3_2_loss = tf.reduce_mean(seg_entropy_loss_valid_256_sn*(mask_256_1_if_proposed*(Nn_256_1_if_proposed/Np_256_1_if_proposed)+1)*(mask_256_1_if_edge*(Nn_256_1_if_edge/Np_256_1_if_edge)+1))
    ...
    #cls_3_2_loss = tf.reduce_mean(input_tensor=cls_entropy_loss*(mask_256_1_if_edge*(Nn_256_1_if_edge/Np_256_1_if_edge)+1))
    #cls_3_2_loss = tf.reduce_mean(input_tensor=cls_entropy_loss*(mask_256_1_if_proposed*(Nn_256_1_if_proposed/Np_256_1_if_proposed)+1))
    cls_3_2_loss = tf.reduce_mean(input_tensor=cls_entropy_loss*(mask_256_1_if_proposed*(Nn_256_1_if_proposed/Np_256_1_if_proposed)+1))
    #cls_3_2_loss = tf.reduce_mean(input_tensor=cls_entropy_loss*((mask_256_1_if_proposed*(Nn_256_1_if_proposed/Np_256_1_if_proposed)+1)*(mask_256_1_if_edge*(Nn_256_1_if_edge/Np_256_1_if_edge)+1)))
    ...
    pred_open_curve_reg_BSpline = tf.cast(tf.reshape(pred_open_curve_reg_BSpline, (num_corner_pairs, 3, 7)), dtype = tf.float32)
    pred_open_curve_reg_BSplines = tf.map_fn(fn = tf_BSpline_function, elems = pred_open_curve_reg_BSpline, dtype=tf.float32, back_prop = True)
    ...
    '''
    open_curve_seg_mask = tf.cast(tf.equal(batch_mask_256_1_if_edge, 1), dtype = tf.float32)
    open_curve_cls_mask_Line = tf.expand_dims(tf.cast(tf.equal(batch_labels_type, 2), dtype = tf.float32), axis = 1)
    Line_mask = tf.multiply(open_curve_seg_mask, open_curve_cls_mask_Line)

    open_curve_cls_mask_BSpline = tf.expand_dims(tf.cast(tf.equal(batch_labels_type, 1), dtype = tf.float32), axis = 1)
    BSpline_mask = tf.multiply(open_curve_seg_mask, open_curve_cls_mask_BSpline)
    '''
    ...
    '''
    #loss_line = tf.reduce_mean(tf.cast(batch_labels_type_one_hot[:, 2], dtype = tf.float32)*tf.reduce_mean(Line_mask*Chamfer_Distance_reg_Lines*(mask_256_sn_if_proposed*mask_256_sn_if_edge(Nn_256_sn_if_edge/Np_256_sn_if_edge)+1)*(mask_256_1_if_edge*(Nn_256_1_if_edge/Np_256_1_if_edge)+1), axis = 1))
    #loss_line = tf.reduce_mean(tf.reduce_mean(Line_mask*Chamfer_Distance_reg_Lines*(mask_256_sn_if_proposed*mask_256_sn_if_edge*(Nn_256_sn_if_edge/Np_256_sn_if_edge)+1)*(mask_256_1_if_edge*(Nn_256_1_if_edge/Np_256_1_if_edge)+1), axis = 1))
    loss_line = tf.reduce_mean(tf.reduce_mean(Line_mask*Chamfer_Distance_reg_Lines*(mask_256_1_if_edge*(Nn_256_1_if_edge/Np_256_1_if_edge)+1), axis = 1))
    #loss_BSpline = tf.reduce_mean(tf.cast(batch_labels_type_one_hot[:, 1], dtype = tf.float32)*tf.reduce_mean(BSpline_mask*Chamfer_Distance_reg_BSplines*(mask_256_sn_if_proposed*(Nn_256_sn_if_proposed/Np_256_sn_if_proposed)+1)*(mask_256_1_if_proposed*(Nn_256_1_if_proposed/Np_256_1_if_proposed)+1), axis = 1))
    #loss_BSpline = tf.reduce_mean(tf.reduce_mean(BSpline_mask*Chamfer_Distance_reg_BSplines*(mask_256_sn_if_proposed*mask_256_sn_if_edge*(Nn_256_sn_if_edge/Np_256_sn_if_edge)+1)*(mask_256_1_if_edge*(Nn_256_1_if_edge/Np_256_1_if_edge)+1), axis = 1))
    loss_BSpline = tf.reduce_mean(tf.reduce_mean(BSpline_mask*Chamfer_Distance_reg_BSplines*(mask_256_1_if_edge*(Nn_256_1_if_edge/Np_256_1_if_edge)+1), axis = 1))
    '''


    #loss_line = tf.reduce_sum(tf.cast(batch_labels_type_one_hot[:, 2], dtype = tf.float32)*tf.reduce_sum(line_mask*Chamfer_Distance_reg_lines*(mask_256_sn*(Nn_256_sn/Np_256_sn)+1), axis = 1)*(mask_1_if_corner*(Nn_pair_1_if_corner/Np_pair_1_if_corner)+1))
    #loss_line = tf.reduce_sum(tf.cast(batch_labels_type_one_hot[:, 2], dtype = tf.float32)*tf.reduce_sum(line_mask*Chamfer_Distance_reg_lines*(mask_256_sn*(Nn_256_sn/Np_256_sn)+1), axis = 1)*(mask_1_if_corner*(Nn_pair_1_if_corner/Np_pair_1_if_corner)+1))
    #loss_line = tf.reduce_sum(pred_open_curve_cls_line_ones*tf.cast(batch_labels_type_one_hot[:, 2], dtype = tf.float32)*tf.reduce_sum(tf.cast(tf.equal(tf.argmax(pred_open_curve_seg, axis = 2), 1), dtype = tf.float32)*Chamfer_Distance_reg_lines*(mask_256_sn*(Nn_256_sn/Np_256_sn)+1), axis = 1)*(mask_1_if_corner*(Nn_pair_1_if_corner/Np_pair_1_if_corner)+1))
    #loss_line = tf.reduce_sum(pred_open_curve_cls_line_ones*tf.cast(batch_labels_type_one_hot[:, 2], dtype = tf.float32)*tf.reduce_sum(*Chamfer_Distance_reg_lines*(mask_256_sn*(Nn_256_sn/Np_256_sn)+1), axis = 1)*(mask_1_if_corner*(Nn_pair_1_if_corner/Np_pair_1_if_corner)+1))
    #loss_line = tf.reduce_sum(tf.nn.softmax(pred_open_curve_cls)[..., 2]*tf.cast(batch_labels_type_one_hot[:, 2], dtype = tf.float32)*tf.reduce_sum(Chamfer_Distance_reg_lines*(mask_256_sn*(Nn_256_sn/Np_256_sn)+1), axis = 1)*(mask_1_if_corner*(Nn_pair_1_if_corner/Np_pair_1_if_corner)+1))
    ...
    #Loss_proposal = W_seg*seg_3_2_loss + mat_diff_loss*reg_weight + W_cls*cls_3_2_loss
    Loss_proposal = W_seg*seg_3_2_loss + W_cls*cls_3_2_loss
    ...
    return Loss_proposal, seg_3_2_loss, cls_3_2_loss
...
def get_stage_1_loss(pred_labels_edge_p, \
    ...
    Np = tf.expand_dims(tf.reduce_sum(mask,axis=1),1) + 0.001
    ...
    Np_1_1 = tf.expand_dims(tf.reduce_sum(mask_1_1,axis=1),1) + 0.001
    ...
    # original
    #reg_edge_3_1_loss = tf.reduce_mean(tf.reduce_sum(tf.cast(labels_edge_p, dtype = tf.float32)*tf.reduce_mean(smooth_l1_dist(pred_reg_edge_p-reg_edge_p),axis=2)*(mask*(Ng/Np)+1), \
    #                        axis = 1)/tf.reduce_sum(mask,axis=1))
    #reg_corner_3_1_loss = tf.reduce_mean(tf.reduce_sum(tf.cast(labels_corner_p, dtype = tf.float32)*tf.reduce_mean(smooth_l1_dist(pred_reg_corner_p-reg_corner_p),axis=2)*(mask_1_1*(Ng_1_1/Np_1_1)+1), \
    ...
    #                        axis = 1))
    # second try
    # reg_edge_3_1_loss = tf.reduce_sum(tf.reduce_mean(smooth_l1_dist(pred_reg_edge_p-reg_edge_p),axis=2)*(mask*(Ng/Np)+1))
    # reg_corner_3_1_loss = tf.reduce_sum(tf.reduce_mean(smooth_l1_dist(pred_reg_corner_p-reg_corner_p),axis=2)*(mask_1_1*(Ng_1_1/Np_1_1)+1))
    
    # third try:
    '''
    reg_edge_3_1_loss = tf.multiply(pred_reg_edge_p-reg_edge_p, pred_reg_edge_p-reg_edge_p)
    reg_edge_3_1_loss_sum = tf.sqrt(tf.reduce_sum(reg_edge_3_1_loss, axis = 2))*(mask*(Ng/Np)+1)
    reg_edge_3_1_loss_total = tf.reduce_mean(reg_edge_3_1_loss_sum)

    reg_corner_3_1_loss = tf.multiply(pred_reg_corner_p-reg_corner_p, pred_reg_corner_p-reg_corner_p)
    reg_corner_3_1_loss_sum = tf.sqrt(tf.reduce_sum(reg_corner_3_1_loss, axis = 2))*(mask_1_1*(Ng_1_1/Np_1_1)+1)
    reg_corner_3_1_loss_total = tf.reduce_mean(reg_corner_3_1_loss_sum)
    '''
    # fourth try:
    reg_edge_3_1_loss = tf.multiply(pred_reg_edge_p-reg_edge_p, pred_reg_edge_p-reg_edge_p)
    reg_edge_3_1_loss_sum = tf.sqrt(tf.reduce_sum(reg_edge_3_1_loss, axis = 2))*(mask*(Ng/Np)+1)
    ...
    #reg_edge_3_1_loss = tf.reduce_mean(tf.reduce_sum(smooth_l1_dist(pred_reg_edge_p-reg_edge_p)*(mask*(Ng/Np)+1),axis=2))
    #reg_corner_3_1_loss = tf.reduce_mean(tf.reduce_sum(smooth_l1_dist(pred_reg_corner_p-reg_corner_p)*(mask_1_1*(Ng_1_1/Np_1_1)+1),axis=2))



    # check for optimal lambda_edge to balance.
    lambda_edge, lambda_corner  = 50.0, 50.0
    # lambda_edge, lambda_corner  = 1.0, 1.0

    # check if these are ok:
    # L_edge = edge_3_1_loss + lambda_edge*reg_edge_3_1_loss
    L_edge = edge_3_1_loss + lambda_edge*reg_edge_3_1_loss_total
    ...
    L_corner = corner_3_1_loss + lambda_corner*reg_corner_3_1_loss_total
    ...
    return edge_3_1_loss, edge_3_1_recall, edge_3_1_acc, corner_3_1_loss, corner_3_1_recall, corner_3_1_acc, reg_edge_3_1_loss_total, reg_corner_3_1_loss_total, loss_31
