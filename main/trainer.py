    ...
    def build_graph_32(self):
        with self.graph_32.as_default():
            with tf.device('/cpu:0'):
                open_gt_corner_pair_sample_points_pl, open_gt_corner_valid_mask_256_64, open_gt_labels_256_64, open_gt_labels_pair = self.MODEL.placeholder_inputs_32(self.BATCH_SIZE)
                ...
                self.MODEL.get_model_32(open_gt_corner_pair_sample_points_pl, is_training_32, bn_decay=self.bn_decay_32)
                ...
                for i in range(self.NUM_GPUS):
                    with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=True):
                        with tf.device('/gpu:%d'%(i)), tf.compat.v1.name_scope('gpu_%d'%(i)) as scope:
                            ...
                            pred_open_curve_seg, end_points = self.MODEL.get_model_32(batch_corner_pair_sample_points_pl, is_training_32, bn_decay=self.bn_decay_32)
                ...
                self.train_ops_32 = self.optimizer_32.apply_gradients(grads, global_step=self.batch_32)
                # Get training operator
        ...
        with self.graph_31.as_default():
            with tf.device('/cpu:0'):
                ...
                if self.STAGE == 1:
                    ...
                    self.total_loss = tf.reduce_mean(input_tensor = total_loss_gpu)
                    ...
                    self.train_ops_31 = self.optimizer_31.apply_gradients(grads, global_step=self.batch_31)
    ...
    def train_graph_32(self):
        ...
        with self.graph_31.as_default():
            ...
            self.train_ops_31 = {'pointclouds_pl': self.pointclouds_pl,
                    ...
                    'train_op': self.train_ops_31,
                    'merged': self.merged,
                    'step': self.batch_31}
        
        print("session 3_1 load end time:", time.time())
        with self.graph_32.as_default():
            print("no....")
        '''
        for epoch in range(self.MAX_EPOCH):
            ...
            # Add summary writers
            with self.graph_32.as_default():
                self.sess_32 = tf.compat.v1.Session(graph = self.graph_32, config=config)
                merged = tf.compat.v1.summary.merge_all()
                self.train_writer_32 = tf.compat.v1.summary.FileWriter(os.path.join(self.LOG_DIR, 'train'), self.sess.graph)
                self.test_writer_32 = tf.compat.v1.summary.FileWriter(os.path.join(self.LOG_DIR, 'test'), self.sess.graph)         
        '''
